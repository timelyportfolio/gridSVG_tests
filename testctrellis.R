#latticeExtra Documentation of c.trellis

testList <- list()
## Combine different types of plots.
testList[[1]] <- c(wireframe(volcano), contourplot(volcano))
...
testList[[2]] <- update(cObj, scales = list(y = list(rot = 0)),
...
testList[[3]] <- c(Sepals = sepals, Petals = petals)

## Force same scales (re-calculate panel limits from merged data):
testList[[4]] <- c(Sepals = sepals, Petals = petals, x.same = TRUE, y.same = TRUE)

## Or - create xyplots from a list of formulas
testList[[5]] <- xyplot.list(list(Sepals = Sepal.Length ~ Sepal.Width,
...
testList[[5]] <- xyplot.list(iris, FUN = histogram)

## Create cumulative distribution plots from a list of objects
testList[[6]] <- xyplot.list(iris[1:4], FUN = qqmath, groups = iris$Species,
            auto.key = TRUE)
...
testList[[7]] <- update(pdboth, xlab = c("Proportion", "Freq"))
...
testList[[8]] <- update(qua, scales = list(at = list(NULL, NULL, NULL, NA),
                          y = list(draw = FALSE)))
...
testList[[10]] <- combo <- c(vmaps, vdens)
## rearrange in pairs
n <- length(vars)
npairs <- rep(1:n, each = 2) + c(0, n)
testList[[11]] <- update(combo[npairs], scales = list(draw = FALSE),
       layout = c(4, 4), between = list(x = c(0, 0.5), y = 0.5))


for(i in 1:length(testList)) {
  print(testList[[i]])
  grid.export(paste0("testctrellis",i,".svg"))
}